...
export function CourseCard({ course }: CourseCardProps) {
  const { updateCourse, addKeyTopic, updateKeyTopic, deleteKeyTopic, deleteResource } =
    useAppState();
  const [expanded, setExpanded] = useState(false);
  const [localNotes, setLocalNotes] = useState(course.notes);
  const [notesFocused, setNotesFocused] = useState(false);

  // Sync external changes (e.g. reset) when the textarea is not focused
  useEffect(() => {
    if (!notesFocused) setLocalNotes(course.notes);
  }, [course.notes, notesFocused]);

  function toggle(e: React.MouseEvent) {
    // Only toggle if the click target is the card root or non-interactive children
  ...
  function handleNotesBlur() {
    ...
    if (localNotes !== course.notes) {
      updateCourse(course.id, { notes: localNotes });
    }
  }

  return (
    <article className={styles.card} onClick={toggle} aria-expanded={expanded}>
          ...
          <div className={styles.buttonGroup}>
            {STATUS_OPTIONS.map(opt => (
              <button
                ...
                className={cn(
                  styles.groupBtn,
                  course.status === opt.value ? styles.groupBtnActive : undefined,
                )}
                onClick={e => {
                  e.stopPropagation();
                  updateCourse(course.id, { status: opt.value });
                }}
              >
                {opt.label}
              </button>
          ...
          <div className={styles.buttonGroup}>
            {RELEVANCE_OPTIONS.map(opt => (
              <button
                ...
                className={cn(
                  styles.groupBtn,
                  course.examRelevance === opt.value ? styles.groupBtnActive : undefined,
                )}
                onClick={e => {
                  e.stopPropagation();
                  updateCourse(course.id, { examRelevance: opt.value });
                }}
              >
                {opt.label}
              </button>
            ...
            <ul className={styles.resourceList}>
              {course.resources.map(resource => (
                <li key={resource.id} className={styles.resourceRow}>
                  ...
                  <Badge label={resource.type} />
                  <button
                    type="button"
                    className={styles.deleteBtn}
                    onClick={e => {
                      e.stopPropagation();
                      deleteResource(course.id, resource.id);
                    }}
                  >
                    Delete
                  </button>
...
function TopicRow({ topic, onCheck, onDelete, onBlur }: TopicRowProps) {
  ...
  // Sync external changes (e.g. reset) when the field is not being edited
  useEffect(() => {
  ...
  return (
    <li className={styles.topicRow}>
      ...
      <input
        ...
        onBlur={() => {
          setFocused(false);
          if (localText !== topic.text) onBlur(localText);
        }}
        onClick={e => e.stopPropagation()}
      />
      <button
